...
// Mesmo padrão das ações do navegador: manuais, locais (LS), CustomEvent pra App.tsx pontear.
...
export interface MapTerritoryWatcher {
  ...
  radius: number;
  sector: string;
  watchTypes: string[]; // ex: ['novos concorrentes', 'reputação', 'fornecedor']
  createdAt: string;
}
...
export interface MapActionEventDetail {
  ...
  payload?: MapFeedCard[] | MapMission | MapCompetitionAnalysis | MapOpportunity | MapComparison | MapTerritoryWatcher | MapRiskAnalysis | MapSectorOpportunity | MapNearbyPartner | MapTerritorySimulation;
}
...
function sectorVoice(sector: string): { audience: string; emphasis: string } {
...
export function dispatchMapAction(detail: MapActionEventDetail): void {
  window.dispatchEvent(new CustomEvent<MapActionEventDetail>('os1:map-action', { detail }));
}
...
export function clearAllMapData(): void {
  Object.values(MAP_LS_KEYS).forEach(k => { try { localStorage.removeItem(k); } catch { /* ignore */ } });
}
